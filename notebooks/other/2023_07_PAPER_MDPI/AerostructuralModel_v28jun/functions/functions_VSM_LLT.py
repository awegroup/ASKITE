    ...
    for kiter in range(Niterations):
        ...
        for ig in range(len(Gamma)):
            if ig in stall and ig>0 and ig<len(alpha)-1:
                GammaNew[ig] = (1-ConvWeight)*Gamma[ig] + ConvWeight*GammaNew[ig]+k2*(Gamma[ig+1]-2*Gamma[ig]+Gamma[ig-1])
            else:
                GammaNew[ig] = (1-ConvWeight)*Gamma[ig] + ConvWeight*GammaNew[ig]
...
def gamma_dist(Gamma,MatrixU,MatrixV,MatrixW,U_2D,airf_coord,data_airf,Uinf,chord,N):
...
def solve_lifting_line_system_newton_approach(ringvec,controlpoints,
                                                           rings,Uinf,Gamma0,data_airf,
                                                           conv_crit,model):
    """
    Solve the VSM or LLM by finding the distribution of Gamma
    ...
    N = len(controlpoints)
            # Look-up airfoil 2D coefficients
    if data_airf.ndim == 3:
        dcl = np.diff(data_airf[:,1,int(N/2)])/np.diff(data_airf[:,0,int(N/2)])
        alphadcl = np.zeros(len(dcl))
        for i in range(len(dcl)):
            alphadcl[i] = 0.5*(data_airf[i,0,int(N/2)]+data_airf[i+1,0,int(N/2)])
    else:
    ...
    Gamma = newton(gamma_dist,Gamma0,args=(MatrixU,MatrixV,MatrixW,U_2D,airf_coord,data_airf,Uinf,chord,N,),tol = 1e-5,maxiter=1000)
...
def plot_geometry(wingpanels,controlpoints,rings,F,coord_L,ax,plot):
    ...
    if plot=='True':
        ...
        for cp in controlpoints:       
            ...
            ax.plot(cp['coordinates_aoa'][0],cp['coordinates_aoa'][1],cp['coordinates_aoa'][2],'#23B52180', marker = '.', markersize = mksize)
        for ring in rings:
            for filament in ring:
                
                if filament['id'] == 'trailing_inf1' or filament['id'] == 'trailing_inf2':
                    coord = np.array([filament['x1'],filament['x1']+filament['dir']*4])
                    ax.plot(coord[:,0], coord[:,1], coord[:,2], '#0D23C233',linewidth = width,linestyle = '--')
                else:
        ...
        for i in range(len(F)):
            a = coord_L[i]
            b = (F[i][0]+F[i][1])/np.linalg.norm(F[int(len(F)/2)][0]+F[int(len(F)/2)][1])*2        
        ...
        legend_elements = [Line2D([0], [0], color='#000000', lw=width*5),
                           Line2D([0], [0], color='#000000', lw=width),
                            Line2D([0], [0], color='#0D23C280', linestyle = '--', lw=width),
                            # Line2D([0], [0], color='#23B521', lw=width),
                            Line2D([0], [0], marker='.', color='w',
                                  markerfacecolor='#23B521',markersize=10
                                  ),
                            Line2D([0], [0], marker='.', color='w',
                                  markerfacecolor='orange',markersize=10)
                    ]    
        
        ax.legend(legend_elements,['Inflatable tubes','Aerodynamic discretization','Horseshoe vortices', 'Lifting Line points ($1/4$c)', 'Control points ($3/4$c)'], frameon = False,
                  loc = 'center')
...
def plot_panel(wingpanels,controlpoints,rings,F,coord_L,ax,plot):
    ...
    if plot=='True':
        ...
        ax.legend(legend_elements,['Inflatable tubes','Aerodynamic discretization','Horseshoe vortices','Local Aerodynamic Forces', 'Lifting Line points ($1/4$c)', 'Control points ($3/4$c)'], frameon = False,
                  loc = 'center')
...
def create_geometry_boxwing(b,h,stagger,chord, Uinf, N,ring_geo,model):
    ...
    for i in range(int(N*0.2)):
        coord[2*i,:] = np.array([-0.25*chord+stagger/h*height[i],b/2,height[i]])
        coord[2*i+1,:] = np.array([0.75*chord+stagger/h*height[i] , b/2, height[i]])
...
def refine_LEI_mesh_ballooning(wingpanels,ball_angle,N_split):
    ...
    for i_sec in range(len(wingpanels)):
        ...
        zvec = (wingpanels[i_sec]['p2']+wingpanels[i_sec]['p1'])/2-(wingpanels[i_sec]['p4']+wingpanels[i_sec]['p3'])/2
