...
vertices_strut = read_obj_file_part(filepath, "strut", "16")[
    0
]  # also negelcted, as it read out as separete ribs
...
def get_planar_in_order(point_list, tol_distance_off_planar):
    ...
    for i in range(
        3, len(point_list)
    ):  # skip the first 2 points as you need 3 to define a surface
        ...
        distance = np.abs(
            np.dot(point_list[i] - point_list[i - 2], cross)
...
left_vertices_LE_planars, LE_other_planar = get_planar_in_order(
    left_vertices_LE, tol_distance_off_planar
)
...
def get_LE_tube_from_planars(
    left_vertices_LE_planars,
    LE_other_planar,
    tol_min_points_for_planar,
    tol_max_points_for_planar,
):
    left_vertices_LE_splitted = []
    left_other_splitted = [LE_other_planar]
    for i in range(0, len(left_vertices_LE_planars)):
        if (
            len(left_vertices_LE_planars[i]) > tol_min_points_for_planar
            and len(left_vertices_LE_planars[i]) < tol_max_points_for_planar
        ):
...
left_vertices_LE_splitted, left_other_splitted = get_LE_tube_from_planars(
    left_vertices_LE_planars,
    LE_other_planar,
    tol_min_points_for_planar,
    tol_max_points_for_planar,
...
def condition_angle_tip_to_mid(
    rib_element,
):  # defining the sorting condition ##TODO: this can also be written as a lambda function
...
get_3D_plot_N_D_triple(
    plot_strut_vertices_idx_0,
    plot_strut_vertices_idx_1,
    plot_strut_vertices_idx_rest,
    "Strut order: red is idx:0, blue is idx:1, black is rest",
)

# %% Splitting off the Canopy Surface and TEs


def get_canopy_surface_from_strut_rib(
    rib, tol_TE_split, tol_perc_tubes, tol_point_distance
):
    ...
    for point in rib_sorted_on_z:
        ...
        if (
            point[2] - rib_sorted_on_z[0][2]
        ) > tol_TE_split:  ##TODO: could change this to planar constraint instead of a defined length, both require input tho...
            flag = False
        ...
        ## if above the TE_circle and below tol_perc_tubes: "0.6" z/|z| (rib length)
            ...
            point[2] < tol_perc_tubes * rib_sorted_on_z[-1][2]
        ):  ##TODO: should ideally be without a hardcoded edge, but not sure how yet
    ...
    if (
        abs(rib_canopy[0][2] - rib_canopy[-1][2])
        < tol_perc_tubes * rib_sorted_on_z[-1][2]
    ):
...
for i in range(0, len(left_vertices_strut_splitted)):
    strut_TE, strut_canopy, strut_other = get_canopy_surface_from_strut_rib(
        left_vertices_strut_splitted[i],
        tol_TE_split,
        tol_perc_tubes,
        tol_point_distance,
    )
    ...
    if len(strut_TE) > 0:
        strut_TE_list.append(strut_TE)
    if len(strut_canopy) > 0:
        strut_canopy_list.append(strut_canopy)
    if len(strut_other) > 0:
        strut_other_list.append(strut_other)
...
get_3D_plot_N_D_4times(
    plot_strut_LE,
    plot_strut_canopy_idx_0,
    plot_strut_canopy_idx_1,
    plot_strut_canopy_idx_rest,
    "Canopy order: red is idx:0, blue is idx:1, black is rest",
)
...
get_3D_plot_N_D_4times(
    ...
    plot_strut_LE,
    plot_strut_canopy,
    "LE: red, covered canopy: green (don't need perfect coverage), TE: blue, rest canopy is black",
)
...
def distance_point_to_plane(point, plane):
    ...
    px3, py3, pz3 = plane[2]
    nx, ny, nz = np.cross(
        (px1 - px2, py1 - py2, pz1 - pz2), (px1 - px3, py1 - py3, pz1 - pz3)
    )
    ax, ay, az = point
    distance = abs((ax - px1) * nx + (ay - py1) * ny + (az - pz1) * nz) / np.sqrt(
        nx**2 + ny**2 + nz**2
    )
    return distance
...
min_y = min(point_i[1] for point_i in left_vertices_wing)
ave_z = (
    max(point[2] for point in left_vertices_canopy)
    + min(point[2] for point in left_vertices_canopy)
) / 2  # chord
...
for i, rib in enumerate(rib_db):
    if (
        rib_db[i][0] == "strut" and len(rib_db[i][2]) == 0
    ):  ##TODO: remove hardcoded occurence of non-filled strut i == 2
        ...
        for point in left_vertices_canopy:
            ...
            if abs(point_angle - LE_centroid_angle) < np.deg2rad(
                tol_angle_canopy_to_strut
            ):
        ...
        for j, point in enumerate(additional_canopy_points):
            ...
            if (
                np.linalg.norm(
                    (lineseg_dists(additional_canopy_points[j], line[0], line[1]))
                )
                < tol_off_line * length_rib_canopy
            ):  ##TODO: hardcoded
                new_canopy.append(
                    np.array(additional_canopy_points[j])
                )  ## append the point to new_canopy
...
get_3D_plot_N_D_4times(
    plot_strut_TE,
    plot_strut_LE,
    plot_strut_canopy_new,
    plot_vertices_canopy,
    "LE: red, new canopy: green (don't need perfect coverage), TE: blue, rest canopy is black",
)
...
for i, rib in enumerate(rib_db):
    ...
    if (
        rib_db[i][0] == "strut" and len(rib_db[i][2]) != 0
    ):  # i != 2: #if the rib is a prefilled strut
        ...
        plane = [
            rib_db[i][2][0],
            rib_db[i][2][int(N_canopy_points / 2)],
            rib_db[i][2][-1],
        ]  # define the plane based on the first, middle and last point of the canopy
        length_rib_canopy = euclidian_distance(
            rib_db[i][2][0], rib_db[i][2][-1]
        )  # length of existing canopy
        new_canopy_points = (
            []
        )  # The newly found points, that will be added to the last canopy points
        ...
        for j, point in enumerate(additional_canopy_points):
            ...
            if (
                abs(distance_point_to_plane(additional_canopy_points[j], plane))
                < tol_off_plane * length_rib_canopy
            ):  ##TODO: hardcoded
                new_canopy_points.append(
                    np.array(additional_canopy_points[j])
                )  ## append the point to new_canopy
...
def find_vector3(vector1, vector2, circle_center, circle_radius, angle):
    ...
    vector3 = circle_center + circle_radius * (
        vector3 - circle_center
    ) / np.linalg.norm(vector3 - circle_center)
...
get_3D_plot_N_D_4times(
    ...
    plot_strut_TE_old,
    plot_strut_LE_old,
    "Should have complete circles now, LE: red, canopy: green, TE: blue",
)
...
for i in range(len(rib_db)):
    if rib_db[i][0] == "strut":
        ...
        canopy_points_to_be_discretized = sorted(
            rib_db[i][2], key=get_angle_TE_to_LE, reverse=True
        )
        ...
        interp_values = get_discretized_canopy(
            np.array(canopy_points_to_be_discretized), [0.25, 0.5, 0.75]
        )  ##TODO: hardcoded..
...
get_3D_plot_N_D_4times(
    ...
    "black shows strut discretisation",
)
...
for mid_tip_LE_i in mid_tip_LE_lst:
    ...
    # (a) sort the canopy point lists from LE to TE
    canopy_points_to_be_discretized = sorted(
        mid_tip_canopy_i, key=get_angle_TE_to_LE, reverse=True
    )
    ...
    mid_tip_right_i[1] = [
        np.array([-point[0], point[1], point[2]]) for point in mid_tip_right_i[1]
    ]
    mid_tip_right_i[2] = [
        np.array([-point[0], point[1], point[2]]) for point in mid_tip_right_i[2]
    ]
    mid_tip_right_i[3] = [
        np.array([-point[0], point[1], point[2]]) for point in mid_tip_right_i[3]
    ]
    mid_tip_right_i[4] = [
        np.array([-point[0], point[1], point[2]]) for point in mid_tip_right_i[4]
    ]
...
plot_mid_tip_LE = [
    ...
    np.concatenate(
        (
            rib_db_mid_tip1_left[1],
            rib_db_mid_tip2_left[1],
            rib_db_mid_tip1_right[1],
            rib_db_mid_tip2_right[1],
        )
    ),
    "red",
    1,
]
plot_mid_tip_particles = [
    2,
    np.concatenate(
        (
            rib_db_mid_tip1_left[4],
            rib_db_mid_tip2_left[4],
            rib_db_mid_tip1_right[4],
            rib_db_mid_tip2_right[4],
        )
    ),
    "black",
    3,
...
for i in range(len(rib_db_right)):
    ...
    if rib_db_right[i][0] == "strut":
        ...
        rib_db_right[i][1] = [
            np.array([-point[0], point[1], point[2]]) for point in rib_db_right[i][1]
        ]
        rib_db_right[i][2] = [
            np.array([-point[0], point[1], point[2]]) for point in rib_db_right[i][2]
        ]
        rib_db_right[i][3] = [
            np.array([-point[0], point[1], point[2]]) for point in rib_db_right[i][3]
        ]
        rib_db_right[i][4] = [
            np.array([-point[0], point[1], point[2]]) for point in rib_db_right[i][4]
        ]
...
plot_mid_tip_LE = [
    3,
    get_rib_db_index_from_rib_db("mid-tip", 1, rib_db_whole),
    "red",
    1,
]
plot_strut_LE = [3, rib_strut_LE, "red", 1]
...
proximity_list = split_for_proximity(
    vertices_bridle, tol_proximity_bridle_vertices, tol_min_points_bridle_knot_or_pulley
)
proximity_list_pulley = split_for_proximity(
    vertices_bridle, tol_proximity_bridle_vertices_pulley, tol_min_points_pulley
)
...
get_3D_plot_N_D_4times_PLUS_lines(
    plot_pulleys, plot_knots, plot_knots, plot_knots, plot_bridle_lines_conn
)
...
for i in range(len(midpoints_surfplan)):
    for j in range(len(midpoints_surfplan_pulley)):
        if (
            euclidian_distance(midpoints_surfplan[i], midpoints_surfplan_pulley[j])
            < tol_proximity_bridle_vertices_is_pulley
        ):
...
for i in range(len(rib_db_whole)):
    ...
    if rib_db_whole[i][0] == "strut" or "mid-tip":
        ...
        new_indices = np.arange(
            idx_counter - len(rib_db_whole[i][4]), idx_counter
        )  # getting the new indices
        ...
        indices_list.append(
            new_indices
        )  ##TODO: used for testing, remove later (test checks out)
...
supervector_bridles = copy.deepcopy(
    midpoints_surfplan
)  # making a copy in case I mess up
...
## DEFINING TOLERANCE
tol_bridle_points_off_line = 0.2  # m
...
for i in range(len(rib_db_whole)):
    if rib_db_whole[i][0] == "strut":  ##TODO: should also do this for the MID ofcourse
        ...
        rib_lower_surface_line = [
            rib_db_whole[i][4][0],
            rib_db_whole[i][4][N_strut_particles - 1],
        ]

        ## 2. Find the indices of the points that lie within a defined proximity of that line
        rib_bridle_indices = []  # storing the indices of the bridle_attachtment points

        for j, bridle_point in enumerate(
            supervector_bridles
        ):  ##TODO: below is hardcoded with 100 distance
            if (
                np.linalg.norm(
                    lineseg_dists(
                        supervector_bridles[j],
                        rib_db_whole[i][4][0],
                        rib_db_whole[i][4][N_strut_particles - 1],
                    )
                )
                < tol_bridle_points_off_line
            ):
        ...
        def cond_dist_to_particle7(rib_bridle_idx):
            point = supervector_bridles[
                rib_bridle_idx - N_wing_particles
            ]  # again correcting for supervector ordering
        ...
        rib_db_whole[i][5] = np.concatenate(
            (rib_db_whole[i][5], rib_bridle_indices), axis=0
        )
    ...
    if rib_db_whole[i][0] == "mid-tip":
        ...
        mid_tip_bridle_connection_idx = (
            0  ##TODO: remove this, just here to make sure it's empty
        )
        ...
        for j, bridle_point in enumerate(supervector_bridles):
            ...
            if (
                euclidian_distance(supervector_bridles[j], rib_db_whole[i][4][0])
                < tol_mid_tip_bridle_point_from_LE_particle
            ):
        ...
        rib_db_whole[i][5] = np.concatenate(
            (rib_db_whole[i][5], [mid_tip_bridle_connection_idx]), axis=0
        )

        # used for test-plotting
        rib_bridle_indices_list.append(
            [mid_tip_bridle_connection_idx]
        )  ##TODO: used for test

## testing if the above worked
rib_bridle_indices_list = np.concatenate(
    rib_bridle_indices_list, axis=0
)  # going to 1D array
...
for i in range(len(rib_db_whole)):
    if rib_db_whole[i][0] == "strut":  ##TODO: should also do this for the MID ofcourse
        ...
        col_ind.append(
            np.append(rib_db_whole[i][5][1:], rib_db_whole[i][5][0])
        ...
        for j in range(N_strut_particles, len(rib_db_whole[i][5])):
            ...
            list_of_possible_connections = rib_db_whole[i][5][
                1 : int(N_strut_particles - 1)
            ]

            # sort the list based on proximity
            proximity_list = sorted(
                list_of_possible_connections, key=cond_dist_to_current_particle
            )
        ...
        if (
            i != 0
            and i != (len(rib_db_whole) - 2 - 4)
            and i < (len(rib_db_whole) - 1 - 4)
        ):
            ...
            row_ind.append(
                np.concatenate(
                    (
                        [rib_db_whole[i][5][-1]],
                        rib_db_whole[i][5][0 : (N_strut_particles - 1)],
                    ),
                    axis=0,
                )
            )
            ...
            col_ind.append(
                np.concatenate(
                    (
                        [rib_db_whole[i + 1][5][-1]],
                        rib_db_whole[i + 1][5][0 : (N_strut_particles - 1)],
                    ),
                    axis=0,
                )
            )
            # the values are all 1's
            conn_indicator.append(
                np.ones(N_strut_particles)
            )  ##TODO: this does not always work, but works for now

    if (
        rib_db_whole[i][0] == "mid-tip"
    ):  ##TODO: should also do this for ALL the MIDs ofcourse
        ...
        row_ind.append(
            np.concatenate(([rib_db_whole[i][5][-1]], rib_db_whole[i][5][:-2]), axis=0)
        )  # (itself) adding the indices of the particles, except for the last one
        ...
        if counter_mid_tip == 0:  # most left mid-tip
            ...
            row_ind.append(
                np.concatenate(
                    ([rib_db_whole[i - 1][5][-1]], rib_db_whole[i - 1][5][:6]), axis=0
                )
            )  ##TODO: hardcoded
            col_ind.append(
                np.concatenate(
                    ([rib_db_whole[i][5][-1]], rib_db_whole[i][5][:6]), axis=0
                )
            )
            ...
            col_ind.append(
                np.concatenate(
                    ([rib_db_whole[1][5][-1]], rib_db_whole[1][5][:6]), axis=0
                )
            )
            conn_indicator.append(np.ones(7))
            ...
            row_ind.append(
                np.concatenate(
                    ([rib_db_whole[12][5][-1]], rib_db_whole[12][5][:6]), axis=0
                )
            )  ##TODO: hardcoded
            col_ind.append(
                np.concatenate(
...
plot_connectivity = [
    supervector,
    np.concatenate((ci_wing, ci_bridle), axis=0),
    np.concatenate((cj_wing, cj_bridle), axis=0),
    "blue",
    2,
]
...
points_ini_model_wing_vertices = transform_coordinate_system_Surfplan_to_Model(
    points_surfplan_wing_vertices
)

# wing particles
points_surfplan_wing = copy.deepcopy(supervector_wing)
points_ini_model_wing = transform_coordinate_system_Surfplan_to_Model(
    points_surfplan_wing
)

# strut LE particles
points_surfplan_LE_per_strut = copy.deepcopy(rib_strut_LE)
points_ini_model_LE_per_strut = [
    transform_coordinate_system_Surfplan_to_Model(points_surfplan_LE_per_strut[i])
    for i in range(len(points_surfplan_LE_per_strut))
]

# mid-tip LE particles
points_surfplan_LE_per_mid = copy.deepcopy(plot_mid_tip_LE[1])
points_ini_model_LE_per_mid_tip = [
    transform_coordinate_system_Surfplan_to_Model(points_surfplan_LE_per_mid[i])
    for i in range(len(points_surfplan_LE_per_mid))
]

# canopy particles
points_surfplan_canopy_per_strut = copy.deepcopy(rib_strut_canopy)
points_ini_model_canopy_per_strut = [
    transform_coordinate_system_Surfplan_to_Model(points_surfplan_canopy_per_strut[i])
    for i in range(len(points_surfplan_canopy_per_strut))
]

# TE particles
points_surfplan_TE_per_strut = copy.deepcopy(rib_strut_TE)
points_ini_model_TE_per_strut = [
    transform_coordinate_system_Surfplan_to_Model(points_surfplan_TE_per_strut[i])
    for i in range(len(points_surfplan_TE_per_strut))
]

# bridle line attachment points
points_surfplan_bridle_attachment_points = copy.deepcopy(rib_bridle_points_list)
points_ini_model_bridle_attachment_points = (
    transform_coordinate_system_Surfplan_to_Model(
        points_surfplan_bridle_attachment_points
    )
)
...
points_ini_model_bridle_vertices = transform_coordinate_system_Surfplan_to_Model(
    points_surfplan_bridle_vertices
)

# bridles particles
points_surfplan_bridle = copy.deepcopy(supervector_bridles)
points_ini_model_bridle = transform_coordinate_system_Surfplan_to_Model(
    points_surfplan_bridle
)
...
points_ini_model_pulleys = transform_coordinate_system_Surfplan_to_Model(
    points_surfplan_pulleys
)
...
for i in range(len(rib_db_whole_surfplan)):
    rib_db_whole_model[i][1] = transform_coordinate_system_Surfplan_to_Model(
        rib_db_whole_surfplan[i][1]
    )
    rib_db_whole_model[i][2] = transform_coordinate_system_Surfplan_to_Model(
        rib_db_whole_surfplan[i][2]
    )
    rib_db_whole_model[i][3] = transform_coordinate_system_Surfplan_to_Model(
        rib_db_whole_surfplan[i][3]
    )
    rib_db_whole_model[i][4] = transform_coordinate_system_Surfplan_to_Model(
        rib_db_whole_surfplan[i][4]
    )
...
plot_points_ini_model_bridle_vertices = [
    2,
    points_ini_model_bridle_vertices,
    "black",
    1,
]
...
get_3D_plot_N_D_4times_PLUS_lines(
    plot_points_ini_model,
    plot_points_ini_model_wing_vertices,
    plot_points_ini_model_bridle_vertices,
    plot_points_ini_model_pulleys,
    plot_conn_model,
)
...
plot_points_ini_model_bridle_attachment_points = [
    2,
    points_ini_model_bridle_attachment_points,
    "orange",
    4.5,
...
plot_conn_bridle = [
    points_ini_model_bridle,
    ci_bridle_indexed_on_bridle,
    cj_bridle_indexed_on_bridle,
    "black",
    1,
]

get_3D_plot_N_D_6times_PLUS_2lines(
    plot_points_ini_model_bridle,
    plot_point_ini_model_strut_LE,
    plot_point_ini_model_mid_tip_LE,
    plot_point_ini_model_strut_canopy,
    plot_point_ini_model_strut_TE,
    plot_points_ini_model_bridle_attachment_points,
    plot_conn_model,
    plot_conn_bridle,
)
...
# wing
np.save(
    filepath + "vertices_wing_model.npy",
    transform_coordinate_system_Surfplan_to_Model(vertices_wing),
)
...
np.save(
    filepath + "points_ini_model_LE_per_mid_tip.npy", points_ini_model_LE_per_mid_tip
)
np.save(
    filepath + "points_ini_model_canopy_per_strut.npy",
    points_ini_model_canopy_per_strut,
)
np.save(filepath + "points_ini_model_TE_per_strut.npy", points_ini_model_TE_per_strut)
np.save(
    filepath + "points_ini_model_bridle_attachment_points.npy",
    points_ini_model_bridle_attachment_points,
)

# bridle line system
np.save(
    filepath + "points_ini_model_bridle_vertices.npy", points_ini_model_bridle_vertices
)
