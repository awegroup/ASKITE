...
def setup_config(config_path):
    ...
    aero = AeroConfig.from_yaml(config_data)
    solver = SolverConfig.create(config_data)
    aero_structural = AeroStructuralConfig.create(config_data)
    tether = TetherConfig.create(config_data)
    kite = KiteConfig.create(config_data)

    # Create Config
    return Config.create(config_data, aero=aero)
