...
def print_forces(
    ...
    print(
        f"F_gravity                       : Fx: {np.sum(force_gravity[:,0]):.2f}N, Fy: {np.sum(force_gravity[:,1]):.2f}N, Fz: {np.sum(force_gravity[:,2]):.2f}N"
    )
    print(f"_________+")
...
def print_results(loaded_data):

    # Unpacking the loaded_data
    points = loaded_data["points"]
    config = loaded_data["config"]
    is_convergence = loaded_data["is_convergence"]
    num_of_iterations = loaded_data["num_of_iterations"]
    aero_structural_total_time = loaded_data["aero_structural_total_time"]
    vel_app = loaded_data["vel_app"]
    residual_f_including_fixed_nodes = loaded_data["residual_f_including_fixed_nodes"]
    residual_f = loaded_data["residual_f"]
    f_internal = loaded_data["f_internal"]
    f_external = loaded_data["f_external"]
    force_aero = loaded_data["force_aero"]
    force_aero_wing = loaded_data["force_aero_wing"]
    force_aero_bridle = loaded_data["force_aero_bridle"]
    f_tether_drag = loaded_data["f_tether_drag"]
    force_gravity = loaded_data["force_gravity"]
    ...
    print_aerostructural(
        is_convergence,
        num_of_iterations,
        config.aero_structural.max_iter,
        aero_structural_total_time,
    )
    print_settings(vel_app, config)
    print_initial_kite_dimensions(config)
    print_forces(
        f_internal,
        f_external,
        force_aero,
    ...
    print_aero(points, vel_app, force_aero_wing, force_aero_bridle, config)
    print_structural(points, bridle_rest_lengths, wing_rest_lengths, config)
