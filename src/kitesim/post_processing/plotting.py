...
def plot_aero(
    ...
    path_results_run_plot_folder = f"{path_run_results_folder}/plots/"
    if not os.path.exists(path_results_run_plot_folder):
        os.makedirs(path_results_run_plot_folder)
    plt.savefig(
        f"{path_results_run_plot_folder}/aero_structural_plot_elev{elev}_azim_{azim}.{plot_format}",
        format=f"{plot_format}",
    )
...
def make_plot(loaded_data, path_run_results_folder):

    # Unpacking the loaded_data
    points = loaded_data["points"]
    vel_app = loaded_data["vel_app"]
    config = loaded_data["config"]
    wingpanels = loaded_data["wingpanels"]
    controlpoints = loaded_data["controlpoints"]
    rings = loaded_data["rings"]
    coord_L = loaded_data["coord_L"]
    F_rel = loaded_data["F_rel"]
    ...
    # loop through the different elevations and azimuths, creating an .svg plot for each
    plot_elev = config.plot_elev
    plot_azim = config.plot_azim

    for elev, azim in zip(plot_elev, plot_azim):
        plot_aero(
            points,
            elongation_values,
            vel_app,
