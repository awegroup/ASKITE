...
def update_for_billowing(points_ini,springL_wing,u_p):
    ...
    def billowing_up(u_p):
        ...
        return np.array([1+ bill_plate_1*(1-u_p), 1+ bill_plate_2*(1-u_p), 1+ bill_plate_3*(1-u_p),1 + bill_plate_4*(1-u_p)])
    ...
    points_ini[14,1] = points_ini[14,1]*(billowing_up(u_p)[0]) # Move points too to avoid asymmetry
    points_ini[15,1] = points_ini[15,1]*(billowing_up(u_p)[0]) # Move points too to avoid asymmetry
...
def find_symmetrical_pairs(points, tolerance=1e-5):
    ...
    for left_index in left_points_indices:
        mirrored_left_point = [points[left_index][0], -points[left_index][1], points[left_index][2]]
        matched_indices = [right_index for right_index in right_points_indices if np.allclose(points[right_index], mirrored_left_point, atol=tolerance)]
        symmetrical_pairs.extend([(left_index, right_index) for right_index in matched_indices])
...
def make_symmetrical(points_ini,points, tolerance=1e-5):
    ...
    for pair_index in pair_indices: #loop through each pair, and make [1] equal to the mirror of [0], using -y coordinate
        point_to_be_mirrored = points[pair_index[0]]
        points[pair_index[1]] = [point_to_be_mirrored[0],-point_to_be_mirrored[1],point_to_be_mirrored[2]] 
