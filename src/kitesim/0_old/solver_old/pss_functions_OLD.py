...
from kitesim.utils import convert_points
...
def instantiate_psystem(PROJECT_DIR, config_dict, config_kite_dict):

    points_ini = convert_points(config_kite_dict["points"])
    ...
    # print(f"extract_plate_point_indices(): {extract_plate_point_indices()}")
    # print(
    #     f'config_kite_dict["plate_point_indices"]: {np.array(config_kite_dict["plate_point_indices"])}'
    # )
    # breakpoint()
    ...
    if config_dict["is_with_initial_plot"]:
        ...
        path_cases = (
            Path(PROJECT_DIR) / "src" / "kitesim" / "cases" / "default_case.yaml"
        )
        kite_name = config_dict["kite_name"]
    ...
    return psystem, pss_param_dict, wing_rest_lengths_initial, m_array
