...
def solve_lifting_line_system_matrix_approach_semiinfinite_new(
    ringvec,
    controlpoints,
    rings,
    Uinf,
...
def calculate_force_aero_wing_VSM(
    ...
    input_VSM: class VSM
        model: str ="VSM" or "LLT"
        n_splits: int = 2 # number of splits of each segment
        billowing_angles: np.ndarray #todo
        ring_geometry: str ="5fil" #"3fil" or "5fil"
        n_segments: int
        tube_diameters: np.ndarray = [0.1,0.2,0.3,] (length 0F n_segments) /chord
        is_tube_diameter_dimensionless: bool = [False,False,..]
        canopy_max_heights: np.ndarray [0.3,0.3] #length OF n_segments /chord
        is_canopy_max_height_dimensionless: bool = [False,False,..]
        cd_multiplier: float = 1.0 #used for old stuff.
        n_iter: int = 1500
        error_limit: 1e-5
        relax_factor: 3e-2
    """
    ...
    ## initialise
    model = input_VSM.model
    n_splits = int(input_VSM.n_splits)

    # Refine mesh, include ballooning
    coord = refine_LEI_mesh_ballooning(
        points_left_to_right, input_VSM.billowing_angles, n_splits + 1
    )
    ...
    data_airf = calculate_polar_lookup_table(
        controlpoints,
        input_VSM.n_segments,
        n_panels_aero,
        n_splits,
        input_VSM.tube_diameters,
        input_VSM.is_tube_diameter_dimensionless,
        input_VSM.canopy_max_heights,
...
#     ## initialise
#     model = input_VSM.aero.model
#     n_split = input_VSM.aero.n_splits

#     # Refine mesh, include ballooning
#     coord = refine_LEI_mesh_ballooning(
#         points_left_to_right, input_VSM.kite.billowing_angles, n_split + 1
#     )
#     n_panels_aero = int(len(coord) / 2) - 1
#     controlpoints, rings, wingpanels, ringvec, coord_L = create_geometry_LEI(
#         coord, vel_app, n_panels_aero + 1, input_VSM.aero.ring_geometry, model
#     )

#     data_airf = calculate_polar_lookup_table(
#         controlpoints,
#         input_VSM.kite.n_segments,
#         n_panels_aero,
#         n_split,
#         input_VSM.kite.airfoil.tube_diameters,
#         input_VSM.kite.airfoil.is_tube_diameter_dimensionless,
#         input_VSM.kite.airfoil.canopy_max_heights,
#         input_VSM.kite.airfoil.is_canopy_max_height_dimensionless,
#     )

#     ## aritificially increasing the drag #TODO: should be removed
#     cd_multiplier = input_VSM.aero.cd_multiplier
#     Niterations = input_VSM.aero.n_iter
#     errorlimit = input_VSM.aero.error_limit
#     ConvWeight = input_VSM.aero.relax_factor
