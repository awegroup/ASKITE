...
def read_obj_file_bridles_faces(file_path):
  ...
  face_pattern = re.compile(r'f\s + (-?\d+)/(-?\d+)/(-?\d+)\s + (-?\d+)/(-?\d+)/(-?\d+)\s + (-?\d+)/(-?\d+)/(-?\d+)\s ')

  bridle_pattern = re.compile(r'g\s+B4_bridle')
  bridle_pattern = re.compile(r'usemtl\s+mtl_18_bridle') # TODO: can probably write this as r'.bridle.'
    ...
    for pointcloud in proximity_list:
        ...
        for point in pointcloud:
            for vertex_index,vertex in enumerate(vertices):
                if np.allclose(point,vertex,rtol=1e-6) and vertex_index not in pointcloud_index:
...
def get_bridle_connectivity(faces_raw,proximity_list_index):
    ...
    for face in faces_raw: #looping through all the faces
        if len(face) == 4: #if the face has 4 vertices
            ...
            for i,pointcloud_index_list in enumerate(proximity_list_index):
                if any(item in pointcloud_index_list for item in face[1:]) and i != pointcloud_a and flag_a == True :
                    pointcloud_b = i
                    flag_b = True
                    break
...
def get_dataframe_bridle(midpoints_surfplan,midpoints_model,bridle_connectivity_i,bridle_connectivity_j):


    bridle_midpoints_Surfplan = midpoints_surfplan
    bridle_midpoints_Model = midpoints_model
    df_surfplan = pd.DataFrame(bridle_midpoints_Surfplan,columns=['x_Surfplan','y_Surfplan','z_Surfplan'])
    df_model = pd.DataFrame(bridle_midpoints_Model,columns=['x_Model','y_Model','z_Model'])
...
def main_bridles(filename, proximity_tol=10,proximity_min_points=5):
    ...
    get_3D_plot_with_lines(midpoints_model,bridle_connectivity_i,bridle_connectivity_j,show_legend=False)

    # (7) Storing the data in a dataframe
    df_bridle,df_bridle_conn = get_dataframe_bridle(midpoints_surfplan,midpoints_model,bridle_connectivity_i,bridle_connectivity_j)
