...
def create_geometry_LEI(coordinates, vel_app, N,RING_GEOMETRY,MODEL):
    ...
    for i in range(N-1):
        ...
        di = np.linalg.norm(coordinates[2*i,:]*chord_3_4+coordinates[2*i+1,:]*chord_1_4-(coordinates[2*i+2,:]*chord_3_4+coordinates[2*i+3,:]*chord_1_4))
        if i == 0:
            diplus = np.linalg.norm(coordinates[2*(i+1),:]*chord_3_4+coordinates[2*(i+1)+1,:]*chord_1_4-(coordinates[2*(i+1)+2,:]*chord_3_4+coordinates[2*(i+1)+3,:]*chord_1_4))
            ncp = di/(di+diplus)
        elif i == N-2:
            dimin = np.linalg.norm(coordinates[2*(i-1),:]*chord_3_4+coordinates[2*(i-1)+1,:]*chord_1_4-(coordinates[2*(i-1)+2,:]*chord_3_4+coordinates[2*(i-1)+3,:]*chord_1_4))
            ncp = dimin/(dimin+di)
        else:
            dimin = np.linalg.norm(coordinates[2*(i-1),:]*chord_3_4+coordinates[2*(i-1)+1,:]*chord_1_4-(coordinates[2*(i-1)+2,:]*chord_3_4+coordinates[2*(i-1)+3,:]*chord_1_4))
            diplus = np.linalg.norm(coordinates[2*(i+1),:]*chord_3_4+coordinates[2*(i+1)+1,:]*chord_1_4-(coordinates[2*(i+1)+2,:]*chord_3_4+coordinates[2*(i+1)+3,:]*chord_1_4))
        ...
        LLpoint = (section['p2']*(1-ncp)+section['p1']*ncp)*chord_3_4 +(section['p3']*(1-ncp)+section['p4']*ncp)*chord_1_4
        VSMpoint = (section['p2']*(1-ncp)+section['p1']*ncp)*chord_1_4 +(section['p3']*(1-ncp)+section['p4']*ncp)*chord_3_4
...
def refine_LEI_mesh(coord,N_sect,N_SPLITS):
    ...
    for i_sec in range(N_sect):
        ...
        for i_spl in range(N_SPLITS):
            temp_coord[2*i_spl] = coord[2*i_sec,:]*(N_SPLITS-i_spl)/N_SPLITS+\
                coord[2*(i_sec+1),:]*(i_spl)/N_SPLITS
            temp_coord[2*i_spl+1] = coord[2*i_sec+1,:]*(N_SPLITS-i_spl)/N_SPLITS+\
...
def struc2aero_old(points_ini,vel_app,n_chordwise_elements,plate_point_indices,BILLOWING_ANGLES,AERO_CONFIG):
    """
    ...
    coord_refined = refine_LEI_mesh_billowing(wingpanels,BILLOWING_ANGLES,N_SPLITS+1)
    ...
    return coord_refined, controlpoints, rings, wingpanels, ringvec, coord_L, n_chordwise_elements_refined, N_segments_refined

def struc2aero(points_ini,vel_app,n_chordwise_elements,plate_point_indices,BILLOWING_ANGLES,AERO_CONFIG):
    """
    This function transform the structural mesh into an aerodynamic mesh
    and prepares the input parameters for the VSM (/LLT)
